...
export default function HousingMapClient({ results }: { results: any }) {
    ...
    console.log("Results array In HousingClient:", resultsArray, "results", results, 'length', resultsArray.length, 'imge_path example', resultsArray[0]?.image_path);
    // Convert results -> markers (using coords lookup)
    const markers = useMemo(() => {
        return resultsArray
            .map((r, idx) => {
                const coords = COORDS_BY_LOCATION[r.location_name];
                if (!coords) return null;
                return { id: `${r.location_name}-${r.plan_name ?? "plan"}-${idx}`, ...coords, ...r };
            })
